...
if (isset($_GET['type']) && $_GET['type'] != '') {
    ...
    if ($type == 'status') {
        ...
        $query = mysqli_query($link, "UPDATE `contact_us` SET `status`='$status' WHERE `id`='$id'") or die('Status Update Query Not Run');
        if ($query) {
        ...
        $query = mysqli_query($link, "DELETE FROM `contact_us` WHERE `id`='$id'") or die('Delete Query Not Run');
        if ($query) {
            echo "<script>window.location.href='blogs.php';</script>";
        }
    }
                            ...
                            <div class="col-lg-12">
                                <div class="table-responsive-sm">
                                    <table id="subct" class="table table-striped projects">
                                        ...
                                        <tbody>
                                            <?php
                                            $data = mysqli_query($link, "SELECT * FROM `contact_us` ORDER BY `id` DESC");
