...
$(document).ready(function () {
  "use strict";
    // Banner video mute/unmute toggle (both desktop and mobile videos stay in sync)
    (function () {
        var toggle = document.getElementById('banner-video-mute-toggle');
        if (!toggle) return;
        var videos = document.querySelectorAll('#banner .banner-video');
        var iconMuted = toggle.querySelector('.icon-muted');
        var iconUnmuted = toggle.querySelector('.icon-unmuted');
        function updateUI() {
            var isMuted = videos.length && videos[0].muted;
            if (iconMuted) iconMuted.hidden = !isMuted;
            if (iconUnmuted) iconUnmuted.hidden = isMuted;
            toggle.setAttribute('aria-label', isMuted ? 'Unmute video' : 'Mute video');
            toggle.setAttribute('title', isMuted ? 'Unmute video' : 'Mute video');
        }
        toggle.addEventListener('click', function () {
            var nextMuted = !videos[0].muted;
            videos.forEach(function (v) { v.muted = nextMuted; });
            updateUI();
        });
        updateUI();
    })();
